...
function AppContent() {
  const { isAuthenticated, isLoading, error, logout, profile } = useAuth();
  ...
  if (isLoading) {
    return (
      <div style={{
        ...
        <p style={{ fontSize: '1.2rem', fontWeight: 600, color: '#2d3436' }}>
          Carregando PDV Mágico Pro...
        </p>
      </div>
    );
  }

  // Error state - mostrar tela de login mesmo com erro
  if (error) {
    console.warn('Auth error, showing login screen:', error);
    // Não retornamos erro, deixamos o fluxo continuar para mostrar a tela de login
  }
